...
func TestMultiTenantSignalIsolation(t *testing.T) {
	...
	require.Eventually(t, func() bool {
		status1, err := flows.Query(ctx1, exec1.WorkflowID())
		if err != nil {
			return false
		}
		status2, err := flows.Query(ctx2, exec2.WorkflowID())
		if err != nil {
			return false
		}
	...
	err = flows.SendSignal(ctx1, exec2.WorkflowID(), "tenant-ready", &TenantSignalPayload{
		TenantID: tenant1,
	...
	err = flows.SendSignal(ctx1, exec1.WorkflowID(), "tenant-ready", &TenantSignalPayload{
		TenantID: tenant1,
		Message:  "ack-tenant1",
	})
	require.NoError(t, err)

	err = flows.SendSignal(ctx2, exec2.WorkflowID(), "tenant-ready", &TenantSignalPayload{
		TenantID: tenant2,
		Message:  "ack-tenant2",
	})
	require.NoError(t, err)
